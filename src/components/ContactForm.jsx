...
export default function ContactForm({ initialService = "", onSuccessSubmit, general, phones, emails }) {
  ...
  const [formData, setFormData] = useState({
    ...
    phone: "",
    company: "",
    serviceType: initialService || "Монтаж лифтов под ключ",
    message: ""
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // POST request using endpoints.contact
  ...
  const addressToShow = getLocalized(general, "address", lang) || "Узбекистан, г. Ташкент, Яккасарайский район, Шота Руставели 45";
  ...
  return (
    <section id="contacts" className="py-20 bg-slate-900 text-white scroll-mt-16 relative overflow-hidden">
      <div className="absolute inset-x-0 top-0 h-40 bg-gradient-to-b from-slate-950/40 to-transparent"></div>
      <div className="absolute -right-32 -top-32 w-96 h-96 bg-primary-950/45 rounded-full filter blur-3xl opacity-60"></div>
          ...
          <div className="lg:col-span-5 flex flex-col justify-between space-y-8 text-left">
            <div className="space-y-4">
              <span className="inline-flex items-center gap-2 bg-primary-950 text-primary-300 border border-primary-800 text-xs font-bold px-3 py-1 rounded-md uppercase tracking-wider">
                📍 {lang === 'uz' ? "Toshkentdagi ofisimiz" : lang === 'en' ? "Our Tashkent Office" : "Наш офис в Ташкенте"}
              </span>
              <h2 className="text-3xl md:text-4xl font-display font-extrabold text-white tracking-tight leading-tight">
                {lang === 'uz' ? "Loyihangizni muhokama\nqilaylik" : lang === 'en' ? "Let's Discuss\nYour Project" : "Давайте обсудим \nваш проект"}
              </h2>
              <p className="text-sm md:text-base text-slate-400 font-sans font-light">
                {lang === 'uz' ? (
                  <>Bizning shourumimizga tashrif buyuring yoki to'g'ridan-to'g'ri ob'yektingizda lift shaxtalarini o'lchash uchun muhandisimiz chaqiruviga buyurtma bering. Telefon va messenjerlar orqali har doim aloqadamiz.</>
                ) : lang === 'en' ? (
                  <>Visit our showroom or order a design engineer dispatch for structural measurements at your construction site. We are always available via call or messaging channels.</>
                ) : (
                  <>Приезжайте к нам в шоурум или заказывайте выезд инженера для замера шахт прямо на строящийся объект. Мы всегда на связи по телефону и в мессенджерах.</>
                )}
              </p>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex gap-4">
                <div className="w-10 h-10 rounded-lg bg-primary-900/60 border border-primary-500/20 text-primary-400 flex items-center justify-center shrink-0 shadow-lg">
              ...
              <div className="flex gap-4">
                ...
                <div>
                  <span className="block text-[11px] uppercase tracking-wider text-slate-400 font-mono">
                    {lang === 'uz' ? "Qabulxona / Aloqa" : lang === 'en' ? "Reception / Hotline" : "Приемная / Авария"}
                  </span>
                  <a href={`tel:${phoneToShow.replace(/[^\d+]/g, '')}`} className="text-sm font-bold text-slate-100 hover:text-amber-500 transition-colors">
                    {phoneToShow}
                  </a>
                </div>
              </div>
              ...
              <div className="flex gap-4">
                ...
                <div>
                  ...
                  <a href={`mailto:${emailToShow}`} className="text-sm font-semibold text-slate-300 hover:text-white transition-colors">
                    {emailToShow}
                  </a>
                </div>
              </div>
            ...
            {/* Map simulator visual */}
            <div className="bg-slate-950/70 border border-slate-800 p-4.5 rounded-2xl space-y-3 shadow-2xl relative select-none">
              <span className="text-[10px] text-slate-500 font-mono block uppercase tracking-wider">
                {lang === 'uz' ? "Toshkent xaritasi" : lang === 'en' ? "Location Map (Tashkent)" : "Карта проезда (Ташкент)"}
              </span>
              <div className="h-28 bg-[radial-gradient(#1e293b_1px,transparent_1px)] bg-[size:10px_10px] border border-slate-800 rounded-xl relative flex flex-col justify-center items-center">
                <MapPin className="w-7 h-7 text-rose-500 fill-rose-500/20 animate-bounce cursor-help" />
                <span className="text-[11px] text-white font-semibold font-display mt-1">Safetech Engineering</span>
                <span className="text-[9px] text-slate-500 font-mono">Шота Руставели / Яккасарай</span>
                <div className="absolute top-2 left-4 w-6 h-0.5 bg-primary-500/30"></div>
                <div className="absolute top-4 left-4 w-0.5 h-6 bg-primary-500/30"></div>
                <div className="absolute bottom-2 right-4 w-6 h-0.5 bg-primary-500/30"></div>
              </div>
            </div>
          </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1">
                      ...
                      >
                        <option value="Монтаж лифтов под ключ">
                          {lang === 'uz' ? "Liftlarni to'liq montaj qilish" : lang === 'en' ? "Turnkey Elevator Installation" : "Монтаж лифтов под ключ"}
                        </option>
                        <option value="Техническое обслуживание (ТО)">
                          {lang === 'uz' ? "Texnik xizmat ko'rsatish (TO)" : lang === 'en' ? "Regular Maintenance (TO)" : "Техническое обслуживание (ТО)"}
                        </option>
                        <option value="Модернизация лифта">
                          {lang === 'uz' ? "Liftlarni modernizatsiya qilish" : lang === 'en' ? "Elevator Modernization" : "Модернизация лифта"}
                        </option>
                        <option value="Установка эскалаторов">
                          {lang === 'uz' ? "Eskalatorlarni o'rnatish" : lang === 'en' ? "Escalator Installation" : "Установка эскалаторов"}
                        </option>
                        <option value="Специальные лифты">
                          {lang === 'uz' ? "Maxsus liftlar" : lang === 'en' ? "Custom & Special Lifts" : "Специальные лифты"}
                        </option>
                        <option value="Экспертиза и диагностика">
                          {lang === 'uz' ? "Ekspertiza va diagnostika" : lang === 'en' ? "Audit & Diagnostic Safety" : "Экспертиза и диагностика"}
                        </option>
                      </select>
                    </div>
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    <textarea 
                      ...
                      placeholder={
                        lang === 'uz' 
                          ? "Bino qavatlari soni, liftlar soni yoki nosozliklar haqida ma'lumot kiriting..." 
                          : lang === 'en' 
                          ? "Specify building height, number of shafts, desired capacity or load parameters..." 
                          : "Укажите этажность здания, количество лифтов или описание дефектов, если требуется ремонт..."
                      }
                      className="w-full bg-slate-900 border border-slate-800 rounded-lg p-2.5 text-xs text-white placeholder-slate-650 focus:outline-none focus:border-primary-500 resize-none font-sans font-light"
                    ></textarea>
                  </div>
                  ...
                  <div className="flex flex-col sm:flex-row gap-4 pt-2 items-center justify-between">
                    ...
                    >
                      {loading 
                        ? (lang === 'uz' ? "Yuborilmoqda..." : lang === 'en' ? "Sending..." : "Отправка...") 
                        : (lang === 'uz' ? "Rasmiy so'rov yuborish" : lang === 'en' ? "Submit Estimate Request" : "Отправить официальный запрос")
                      }
                    ...
                    <a
                      ...
                      className="bg-sky-500/10 border border-sky-500/25 hover:bg-sky-500/20 text-sky-400 text-xs py-3 px-4.5 rounded-lg text-center font-semibold flex items-center justify-center gap-1.5 w-full sm:w-auto"
                    >
                  ...
                  <div className="space-y-1 max-w-md mx-auto">
                    ...
                    <p className="text-xs text-slate-400 leading-relaxed font-sans font-light">
                      {lang === 'uz' ? (
                        ...
                        <>Спасибо за обращение. Наш дежурный инженер-конструктор Оргкомитета свяжется с Вами по указанному номеру телефона для уточнения чертежей и составления ТЗ в кратчайшие сроки.</>
                      )}
                    </p>
                  </div>
                  <button
